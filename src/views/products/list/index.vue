  ...
  <div class="app-container">
    <div class="filter-container">
      ...
      <div class="searchBox">
        ...
        <el-select v-model="listQuery.cate" placeholder="類別" clearable class="filter-item" style="width: 130px">
          <el-option v-for="item in cate" :key="item.id" class="filter-item" :label="item.name" :value="item.name" />
        </el-select>
    ...
    >
      ...
      <el-table-column label="圖片預覽" width="220px" align="center" class-name="img-wrapper">
        <template slot-scope="scope">
          <img :src="scope.row.images[0].url" :alt="scope.row.title" style="width: 100%">
        </template>
      </el-table-column>
      <el-table-column label="商品名稱" min-width="300px">
        <template slot-scope="{row}">
          <p class="link-type" @click="handleUpdate(row)">{{ row.title }}</p>
          <el-tag>{{ row.cate }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="規格" width="120px" align="center">
        <template slot-scope="scope">
          <span class="link-type" @click="handleUpdate(scope.row)">{{ scope.row.spec }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" @close="resetTemp()">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="right" label-width="80px" style="width: auto">
        <div>
          ...
          <el-form-item ref="uploadForm" label="圖片上傳" prop="file">
            <el-upload
              class="el-update"
          ...
          <el-form-item label="類別選擇" prop="cate">
            <el-select v-model="temp.cate" class="filter-item" placeholder="類別選擇">
              <el-option v-for="(item, idx) in cate" :key="idx" :label="item.name" :value="item.name" />
            </el-select>
          </el-form-item>
...
import { fetchList, createProduct, updateProduct, deleteProduct, fetchCate } from '@/api/products'
  ...
  data() {
    return {
      ...
      rules: {
        ...
        cate: [{ required: true, message: '尚未選擇類別！', trigger: 'change' }],
